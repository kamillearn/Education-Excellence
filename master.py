...
def load_teachers_data(Year):
    ...
    file_path = fr"C:\Users\moham\neuefische\capstone_project_reform\data\Bildungsdaten\SKL_Teil_Z_Zusammenfassung_{Year}.xlsx"
...
def load_ausgaben_data(year):
    # Construct the file path
    file_path = fr"C:\Users\moham\neuefische\capstone_project_reform\data\Bildungsausgaben\Ausgaben_{year}.xlsx"
    ...
    ausgaben_1 = ausgaben_1.rename(columns={
        ausgaben_1.columns[1]: 'Allg bildende Schulen',
    ...
    ausgaben_2 = ausgaben_2.rename(columns={
        ausgaben_2.columns[1]: 'Grund schulen',
    ...
    ausgaben_3 = ausgaben_3.rename(columns={
        ausgaben_3.columns[1]: 'Personalausgaben',
    ...
    # Apply lambda function to remove spaces and handle non-numeric characters
    for col in ausgaben.columns[1:]:
        ausgaben[col] = ausgaben[col].astype(str).str.replace(' ', '').str.replace('/', '0').str.replace('-', '0')

    # Convert all numeric values to float to be able to convert to int
    for col in ausgaben.columns[1:15]:
        ausgaben[col] = pd.to_numeric(ausgaben[col], errors='coerce').astype('float')
    ...
    for col in ausgaben.columns[1:15]:
        ausgaben[col] = ausgaben[col].astype('Int64')

    # apply lambda function to remove spaces
#    ausgaben.iloc[:, 1:] = ausgaben.iloc[:, 1:].apply(lambda x: str(x).replace(' ', ''))

#    # Replace forward slashes with empty strings
#    ausgaben = ausgaben.apply(lambda x: x.replace('/', '0'))
#    ausgaben.iloc[:, 1:] = ausgaben.iloc[:, 1:].apply(lambda x: x.replace('-', '0'))

#    # convert all numeric values to float to be able to convert to int
#    ausgaben.iloc[:, 1:15] = ausgaben.iloc[:, 1:15].astype('float')
#    ausgaben['column_name'] = ausgaben['column_name'].str.replace(r'\D', '', regex=True).astype(float)
...
def load_exams_data(year):
    # Construct the file path
    file_path = fr"C:\Users\moham\neuefische\capstone_project_reform\data\Abiturnoten\Aus_Abiturnoten_{year}\Noten-Table 1.csv"
...
avg_noten_2023 = load_avg_noten_data(2023)
...
noten_2023 = load_noten_data(2023)
...
exams_2023 = load_exams_data(2023)
...
teachers_2022 = load_teachers_data(2022)
...
students_2022 = load_students_data(2022)
...
students_per_teacher_2022 = load_students_per_teacher_data(2022)
...
hours_per_student_2022 = load_hours_per_student_data(2022)
...
ausgaben_2022 = load_ausgaben_data(2022)


# Concatenating teacher data from 2011 to 2022
teachers_concat = pd.concat([
    teachers_2011, teachers_2012, teachers_2013, teachers_2014, teachers_2015, 
    teachers_2016, teachers_2017, teachers_2018, teachers_2019, teachers_2020, 
    teachers_2021, teachers_2022
], ignore_index=True)

# Concatenating student data from 2011 to 2022
students_concat = pd.concat([
    students_2011, students_2012, students_2013, students_2014, students_2015, 
    students_2016, students_2017, students_2018, students_2019, students_2020, 
    students_2021, students_2022
], ignore_index=True)

# Concatenating students per teacher data from 2011 to 2022
students_per_teacher_concat = pd.concat([
    students_per_teacher_2011, students_per_teacher_2012, students_per_teacher_2013, 
    ...
    students_per_teacher_2020, students_per_teacher_2021, students_per_teacher_2022
], ignore_index=True)

# Concatenating hours per student data from 2011 to 2022
hours_per_student_concat = pd.concat([
    hours_per_student_2011, hours_per_student_2012, hours_per_student_2013, 
    ...
    hours_per_student_2020, hours_per_student_2021, hours_per_student_2022
], ignore_index=True)

# Concatenating ausgaben data from 2010 to 2022
ausgaben_concat = pd.concat([
    ausgaben_2010, ausgaben_2011, ausgaben_2012, ausgaben_2013, ausgaben_2014, 
    ausgaben_2015, ausgaben_2016, ausgaben_2017, ausgaben_2019, ausgaben_2020, 
    ausgaben_2021, ausgaben_2022
], ignore_index=False)

# Concatenating exams data from 2010 to 2023
exams_concat = pd.concat([
    exams_2010, exams_2011, exams_2012, exams_2013, exams_2014, exams_2015, 
    exams_2016, exams_2017, exams_2018, exams_2019, exams_2020, exams_2021, 
    exams_2022, exams_2023
], ignore_index=True)

# Update state names in the exams data
exams_concat = update_states_name.update_names(exams_concat)

# Concatenating noten data from 2010 to 2023
noten_concat = pd.concat([
    noten_2010, noten_2011, noten_2012, noten_2013, noten_2014, noten_2015, 
    noten_2016, noten_2017, noten_2018, noten_2019, noten_2020, noten_2021, 
    noten_2022, noten_2023
], ignore_index=True)

avg_noten_concat = pd.concat([
    avg_noten_2010, avg_noten_2011, avg_noten_2012, avg_noten_2013, avg_noten_2014, avg_noten_2015, 
    avg_noten_2016, avg_noten_2017, avg_noten_2018, avg_noten_2019, avg_noten_2020, avg_noten_2021, 
    avg_noten_2022, avg_noten_2023
], ignore_index=True)